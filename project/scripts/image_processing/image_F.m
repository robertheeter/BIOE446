...
dapi_file = 'DAPI/DAPI_F.tif';
mrna_file = 'mRNA/mRNA_F.tif';
...
xlabel('Nascent mRNA')
ylabel('Free mRNA')
title('Image F: Nascent mRNA vs. Free mRNA Per Cell')
...
function [areas, labels, num_objects, num_nuclei, dapi_img_2] = isolate_nuclei(dapi_img)
    ...
    threshold = adaptthresh(filtered_dapi, 0.62);
    ...
    figure
    imshow(dapi_img_2)
    title('Image F: Segmented DAPI Image')
...
function [num_spots, intensities] = isolate_mrna(dapi_binary_img, mrna_img, nucleus_mask)
    ...
    % figure
    % imshow(mrna_binary)
    % title('Image F: Binary smFISH Image')
