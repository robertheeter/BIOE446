...
dapi_file = 'DAPI/DAPI_E.tif';
mrna_file = 'mRNA/mRNA_E.tif';
...
xlabel('Nascent mRNA')
ylabel('Free mRNA')
title('Image E: Nascent mRNA vs. Free mRNA Per Cell')
...
function [areas, labels, num_objects, num_nuclei, dapi_img_2] = isolate_nuclei(dapi_img)
    ...
    figure
    imshow(dapi_img_2)
    title('Image E: Segmented DAPI Image')
...
function [num_spots, intensities] = isolate_mrna(dapi_binary_img, mrna_img, nucleus_mask)
    ...
    % figure
    % imshow(mrna_binary)
    % title('Image E: Binary smFISH Image')
